...
public class MyBatisGeneratorConfigurationParser {
    ...
    /* modify here */// 像 spring 一样可以接受默认值 ${prop:default}
        ...
        if (newString != null) {
            ...
            while (start > -1 && end > start) {
                ...
                if (propName.contains(":")) {
                    defaultValue = propName.substring(propName.indexOf(":") + 1);
                    propName = propName.substring(0, propName.indexOf(":"));
                }
                String propValue = resolveProperty(propName);
                if (propValue != null || defaultValue != null) {
                    newString = prepend + (propValue == null ? defaultValue : propValue) + append;
                }
